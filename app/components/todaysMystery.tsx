...
export const TodaysMystery=({gutt}:{gutt:RosaryOfTheDay[]})=>{
    ...
    return(
        <>
            <div className=" text-orange-600  h-2 my-8" ></div>
        <h2 className=" text-3xl mt-8">Aicho gutt: {data.title}</h2>
        <div className=" text-orange-600 h-2 my-8" ></div>
        <h3 className="mt-8 text-2xl ">Dis: {mapDayToKonkani[getDay().toLowerCase()]} / <small> {getDay()}</small></h3>
        {
            data.mysteries.map((item,index)=>(
                ...
                <div className=" text-orange-600  h-2 my-8 w-1/2" ></div>
                </>
            ))
        }
